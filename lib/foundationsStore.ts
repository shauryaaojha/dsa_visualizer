// Foundations player store (zustand). Same pattern as every structure store.

import { create } from "zustand";
import { runFoundationsOperation, FOUNDATIONS_OPERATIONS } from "@/engines/foundationsEngine";
import type { FoundationsOperationId, FoundationsProgram, FoundationsStep } from "@/types/visualization";

const SPEED_OPTIONS = [0.5, 1, 1.5, 2, 4] as const;
type Speed = (typeof SPEED_OPTIONS)[number];
// Extra roomy: foundations narration is aimed at first-time programmers.
...
interface FoundationsState {
  ...
  params: FoundationsParams;
  program: FoundationsProgram | null;
  stepIndex: number;
  isPlaying: boolean;
  speed: Speed;

  currentStep: () => FoundationsStep | null;

  setOperation: (op: FoundationsOperationId) => void;
  setParams: (p: Partial<FoundationsParams>) => void;
  run: (op?: FoundationsOperationId, params?: Partial<FoundationsParams>) => void;
...
export const useFoundationsStore = create<FoundationsState>((set, get) => {
...
export { FOUNDATIONS_OPERATIONS, SPEED_OPTIONS };
